...
def interpolate(data, size=None):
    if not size:
        size = (32, 32)
    return np.array(Image.fromarray(data).resize(size, resample=Image.Resampling.BICUBIC))
'''
...
def pre_process(path, y_n):
    ...
    for vector in data:
        for band in range(0, 4):
            ...
            #data_2D_temp_interp = np.array([interpolate(signal) for signal in data_2D_temp])
            # print("data_2d_temp_interp shape:", data_2D_temp_interp.shape)
            data_3D = np.vstack([data_3D, data_2D_temp])
            #print("data_3D shape", data_3D.shape)
            #data_3D_interp = np.array([interpolate(signal) for signal in data_3D])
            #print("data_3D shape", data_3D_interp.shape)
    data_3D = data_3D.reshape(-1, 4, 8, 9)
    print("final data shape:", data_3D_interp.shape)
    return data_3D_interp, arousal_labels, valence_labels


if __name__ == '__main__':
    dataset_dir = "\\DEAP\\DE_1D\\"
    use_baseline = "yes"
    if use_baseline == "yes":
        result_dir = "\\DEAP\\DE_3D\\"
        if os.path.isdir(result_dir) == False:
            os.makedirs(result_dir)
    else:
        resullt_dir = "\\DEAP\\DE_3D\\"
        if os.path.isdir(result_dir) == False:
            os.makedirs(result_dir)
