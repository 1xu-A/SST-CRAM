...
def train(x_all_subject, y_a_all_subject, y_v_all_subject, all_subject_id,
    ...
    np.random.seed(seed)
    loo = LeaveOneOut()
    for fold, (train, test) in enumerate(loo.split(x_all_subject, y_a_all_subject.argmax(1))):

        print(f'\n\nFold {fold + 1}/{len(x_all_subject)}\n\n')
        # if fold > 1:
        #    continue
        K.clear_session()
        ...
        print("x_train number", x_train.shape)
        ...
        print("x_test number", x_test.shape)
        ...
        if fine_tuning:
            ...
            for i in range(subject_n):
                ...
                model.compile(loss=[keras.losses.categorical_crossentropy, keras.losses.categorical_crossentropy],
                              optimizer=tf.keras.optimizers.Adam(learning_rate=0.0001 / 8),
                              metrics=['accuracy'])
                ...
                model.compile(loss=keras.losses.categorical_crossentropy, loss_weights=[1, 0],
                              optimizer=tf.keras.optimizers.Adam(learning_rate=0.0001 / 4),
                              metrics=['accuracy'])
